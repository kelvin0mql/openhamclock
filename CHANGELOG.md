...
- **Open-Meteo Throttle Queue** — Outbound Open-Meteo requests now processed through a rate-limited queue at 4 req/s (240/min, well under 600/min free tier limit). Queue auto-pauses during 429 backoff. Requests waiting >60s are dropped
- **Server-Side Weather Worker** — Weather endpoint no longer triggers upstream fetches. Instead, client requests register their 1° grid cell as "active", and a background worker cycles through all active cells every 2 minutes, refreshing cache at a steady controlled rate. With ~300 unique cells for 2000 users, a full refresh takes ~75 seconds — well within the 2-hour cache TTL. Worker sorts US/NWS cells first (fast, unlimited), then international by most-recently-requested. Cells not requested in 4 hours are pruned. On first request for a new cell (cache miss), the worker is triggered immediately so data appears within seconds. Clients read cache only — zero upstream pressure from HTTP traffic regardless of user count
...
- **Weather still overwhelmed at 2000+ users** — Even with NWS for US, international DX targets generated hundreds of unique coordinates bursting simultaneously. Architectural redesign: (1) weather endpoint is now cache-only — client requests never trigger upstream fetches; (2) background worker owns all upstream fetches, cycling through active grid cells at a controlled steady rate every 2 minutes; (3) coordinate grid coarsened from 0.1° to 1° whole degrees (~111km — weather doesn't vary within this); (4) Open-Meteo requests throttled through queue at 4 req/s; (5) client debounce absorbs rapid DX target changes; (6) HTTP 202 response for uncached cells, client retries in 5s to pick up worker-fetched data
